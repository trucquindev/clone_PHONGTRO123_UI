    ...
    <div className=' w-3/5 flex flex-col justify-center items-center gap-4 bg-white rounded-md shadow-md p-4'>
      ...
      <div className='flex justify-around items-center w-full'>
        {text.statisticals.map((item,index)=>{
            return (
                <div key={index} className='flex flex-col justify-center items-center text-[12px]'>
      ...
      <h3 className='text-lg font-bold text-[18px] py-2 '>{text.priceDescription}</h3>
      <div className='flex items-center justify-center'>
        {start.map((item,index)=>{
            return (
                <span key={index}>
                    <MdOutlineStar size={24}className='text-[20px] text-yellow-400'/>
                </span>
            )
        })}
      </div>
      <p className='text-gray-600 italic text-center text-[13px]'>{text.comments}</p>
